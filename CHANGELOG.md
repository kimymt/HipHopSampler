...
### Fixed
- **2 本指で同時にパッドを叩くと長押しせずに編集画面が開いていた問題**: PR #50 のタイマー実装が単一の useRef だったため、2 つ目のパッドを触った瞬間に 1 つ目のタイマー参照が上書きされ、1 つ目のパッドを離した時に「2 つ目」のタイマーが取り消され、1 つ目のタイマーが孤児として 350ms 後に発火 → 触ったつもりのないパッドの編集画面が勝手に開いていた。  
  ドラムサンプラーで両手叩きは普通の操作なのに考慮漏れだった。タイマー追跡を `Map<padId, Timer>` と `Set<padId>` に変更し、padId ごとに独立して管理。同時押し / 連続タップ / 長押し + 別パッドタップの全パターンが正しく動作。  
  Vitest でマルチタッチを含む 5 ケースのリグレッションテストを追加 (vitest 122 → 127 件)。
