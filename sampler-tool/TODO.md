...
- [ ] **WAV エクスポート** (OfflineAudioContext) — ⚠️ **リファレンスモード関連の必須ガードレールあり**。下記「エクスポート機能の必須ガードレール」を実装着手前に必読。
- [ ] **MIDI 入力対応** (Web MIDI API)
- [ ] **クラウド保存** (要バックエンド)

---

## ⛔️ エクスポート機能の必須ガードレール (リファレンスモード関連)

> **読まずに実装したら法的リスクが出る。エクスポート機能 (WAV / MP3 / MIDI / プロジェクトファイルなど、何らかの形で外部に持ち出せる出力) に着手する前に必ずこのセクションを読み、テストで検証してから merge すること。**

リファレンスモード (v0.5.0.0 で導入) は「ユーザーが好きな曲を読み込んで構造を学ぶ」機能。私的利用を厳守する前提で、ファイルバイト列はブラウザ内に留め保存パスは存在しない。**しかし、エクスポート機能を後付けで実装した瞬間、リファレンス由来のデータが流出する経路が新たに開く可能性がある。** 以下 2 点を **必須実装事項** とする。

### 要件 1: リファレンス由来データのエクスポート不可確認

エクスポート機能の実装直後、テストで以下を全て検証すること:

- [ ] リファレンスモードで読み込んだ AudioBuffer 自体が **WAV/MP3 などのオーディオエクスポート出力に一切含まれない** こと
- [ ] リファレンスから抽出された **波形データ / スペクトログラム / 切り出しサンプル** がエクスポート出力に含まれないこと
- [ ] ユーザーがリファレンスを聴きながらパッドを叩いて録音した場合でも、**録音バッファ内にリファレンスの音声成分が混入しない** ようルーティング上分離されていること (`useReferenceTrack` の AudioBuffer は専用 GainNode 配下、メインミックスから完全切り離す)
- [ ] プロジェクトファイル (JSON 等) には **リファレンス由来の BPM 値とビート位置配列のみ** 含めて良い (これは派生的な数値データ。原曲再構成不能)。**ファイル名・著作者・サムネイル等のメタデータも含めない**
- [ ] 上記を担保する単体テスト + 統合テスト + 「悪意あるユーザーがエクスポートを試みた場合の防御テスト」を作成

### 要件 2: ユーザー同意ゲート (拒否時はブロック)

エクスポート実行時、以下のフローを **ハードコード強制** すること:

1. エクスポートボタンタップ → 同意ダイアログを表示 (エクスポート前は必ず通る、スキップ不可)
2. ダイアログ本文 (例文):
   > **このエクスポートにリファレンスモードで読み込んだ楽曲の音声データは含まれていません。**
   >
   > リファレンスモードは私的学習目的の機能です。書き出されるファイルにはあなたが作ったビート・パターン・サンプルのみが含まれ、リファレンスの音源や派生波形は一切含まれません。
   >
   > エクスポートしたファイルを公開・配布・収益化しても、リファレンス楽曲の著作権を侵害しないことを確認しましたか?
3. ユーザーは **「確認した・同意する」/「同意しない」** の 2 択。
4. **「同意しない」を選んだ場合、エクスポート処理は実行せず、ダイアログを閉じる**。リトライ可能。
5. 同意したセッションの記録 (localStorage に同意済みフラグ) は **しない**。**毎回確認させる**。理由: 一度の同意で「ずっと同意」にすると、後から考えが変わったとき UI が出ないためうっかり流出する。
6. ダイアログ自体に「再表示しない」のオプトアウトボタンを **置かない**
...
const handleExportClick = async () => {
  const ack = await showExportConsentDialog();
  if (!ack) return; // 同意なしならアボート
  await runExport(); // リファレンスは含まない経路
};
```

`showExportConsentDialog` は約束 (Promise) ベースの async モーダルとして実装。Tour と同じ Bottom Sheet パターンが流用可能。

### 検証ガイド

以下のテストケースを 100% パスさせること:

| ケース | 期待動作 |
|---|---|
| リファレンスを読み込まず通常エクスポート | 同意ダイアログ → 同意 → 正常エクスポート |
| リファレンスを読み込み中にエクスポート | 同じ同意フローを通ることを確認 |
| 同意ダイアログで「同意しない」 | エクスポート処理は呼ばれない (`exportFn` のスパイで検証) |
| 同意ダイアログを ESC / 背景タップで閉じる | 「同意しない」と同等扱い、エクスポートしない |
| エクスポート実行後にエクスポートファイルを WAV decoder にかける | リファレンス由来の周波数成分が混入していないこと (波形比較テスト) |

### ファイル

- 同意ダイアログコンポーネント: `src/components/ExportConsentDialog.tsx` (新規) を想定
- エクスポート関数: `src/utils/exportAudio.ts` (新規) を想定 — 内部でリファレンスバスを **物理的に接続しない**
- リファレンス分離テスト: `src/utils/exportAudio.test.ts`
