...
**実装内容:**
- `src/effects/presetDictionary.ts` で日本語/英語キーワード → JSON のマッピング 30件程度
- 「水中」「ローファイ」「広い」「狭い」「歪み」「アシッド」「ヴィンテージ」「クリア」「テレフォン」 etc.
- 入力欄でユーザーが入力 → keyword match → `setFx({ type, wet, param })` 即実行
- マッチしないときは「近い候補」を 3つ提示 (sub-string + Levenshtein distance)

**メリット:**
- コスト 0 / 永久無料
- レイテンシ < 10ms
- iOS Safari でも完全動作
- LLM 障害時の fallback としても使える (Phase 2B/2C 導入後も基盤として残す)

**デメリット:**
- 想定外の表現 (例:「ピヨピヨして」) はマッチしない → Phase 2B/2C で補完
...
1. **Phase 2A (1週間)**: キーワード辞書 30件 + 入力 UI + 候補提示
2. **Phase 2B (2週間、Phase 2A 完了後)**: WebLLM 統合、Qwen 2.5 0.5B、オプトインフロー
3. **Phase 2C (任意、半日)**: BYO key 入力欄 + Gemini Flash 直接呼出
